 ...
 */
Vimeo.prototype.tusUpload = function (path, complete_callback, progress_callback, error_callback, filename) {
  var _self = this;
  ...
  this.request(options, function(err, ticket, status) {
    if (err) {
    ...
    upload.url = ticket.upload_link_secure;
    upload.start();
  });
  return;
};
